...
if ($server->parseSearchtext($_REQUEST["searchtext"])) {
  $url = ADRESSSUCHE_URL . http_build_query(
      array(
  ...
  foreach ($json->response->docs AS $doc) {
    ...
    if (!empty($doc->poi_titel)) {
      $result .= str_replace("#ZUSATZ#", $doc->zusatz, str_replace("#NAME#", $doc->poi_titel, str_replace("#BBOX#", $bbox, str_replace("#BBOXS#", implode(",", $bbox_array), RES_POI)))
      );
      $json_result[] = array("label" => $doc->poi_titel . " " . $doc->zusatz, "bbox" => $bbox_array);

      // STADTTEIL
    } else if (!empty($doc->stadtteil_name)) {
      $result .= str_replace("#ZUSATZ#", $doc->zusatz, str_replace("#NAME#", $doc->stadtteil_name, str_replace("#BBOX#", $bbox, str_replace("#BBOXS#", implode(",", $bbox_array), RES_STADTTEIL)))
      );
      $json_result[] = array("label" => $doc->stadtteil_name . " " . $doc->zusatz, "bbox" => $bbox_array);
      // STRASSE
    } else if (!empty($doc->strasse_name)) {
      $result .= str_replace("#ZUSATZ#", $doc->zusatz, str_replace("#NAME#", $doc->strasse_name, str_replace("#BBOX#", $bbox, str_replace("#BBOXS#", implode(",", $bbox_array), RES_STRASSE)))
      );
      $json_result[] = array("label" => $doc->strasse_name . " " . $doc->zusatz, "bbox" => $bbox_array);
      // ADRESSE
    } else {
      $result .= str_replace("#ZUSATZ#", $doc->zusatz, str_replace("#NAME#", $doc->strasse . " " . $doc->hausnummer . $doc->hausnummerzusatz, str_replace("#BBOX#", $bbox, str_replace("#BBOXS#", implode(",", $bbox_array), RES_ELSE)))
      );
